...
export default function Gestao_eventos() {
  ...
  async function handlePesquisarLocal() {
    ...
    if (!local.trim()) {
      setFeedbackMsg({ texto: 'Introduz um local antes de pesquisar.', classe: 'ge-erro' });
      return;
    }
    if (!dataEvento) {
    ...
    try {
      ...
      const meteoRes = await geObterMeteo(lat, lon);
      const dias = meteoRes.daily.time;
